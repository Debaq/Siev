...
class VideoThread(QThread):
    ...
    def __init__(self, camera_id=2):
        ...
        # Cargar el modelo YOLO entrenado con imágenes de threshold
        self.model = YOLO('best_thr.pt')
    ...
    def process_pupil(self, data):
        try:
            eye_thresh, eye_color, ex, ey, ew, eh, is_first_eye = data
            
            # Buscar contornos en la imagen thresholdeada
            ...
            if contours:
                # Obtener círculo mínimo que encierra el contorno más grande (pupila)
                (cx, cy), radius = cv2.minEnclosingCircle(contours[0])
                
                # Crear máscara en color para visualización
                mask = cv2.cvtColor(eye_thresh, cv2.COLOR_GRAY2BGR)
                
                return (int(cx), int(cy), int(radius), ex, ey, ew, eh, mask)
                
            return None
        except Exception as e:
            print(f"Error en process_pupil: {e}")
            return None
    ...
    def run(self):
        try:
            ...
            self.thresh = np.empty((h, w), dtype=np.uint8)
            ...
            while self.running:
                ...
                # Convertir a gris para threshold
                cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY, dst=self.gray)
                
                # Aplicar ROI
                roi_gray = self.gray[roi_y:roi_y+roi_height, :]
                roi_frame = frame[roi_y:roi_y+roi_height, :]
                
                # Aplicar threshold a la ROI
                # Usamos un threshold medio que funcionará bien con el modelo entrenado
                _, roi_thresh = cv2.threshold(roi_gray, 25, 255, cv2.THRESH_BINARY_INV)
                
                # Redimensionar ROI con threshold para YOLO
                small_thresh = cv2.resize(roi_thresh, None, fx=scale_factor, fy=scale_factor)
                
                # Convertir threshold a BGR para YOLO (si es necesario)
                thresh_bgr = cv2.cvtColor(small_thresh, cv2.COLOR_GRAY2BGR)
                
                # Detectar ojos con YOLO en la ROI thresholdeada
                results = self.model(thresh_bgr, conf=0.5, verbose=False)
                ...
                for i, r in enumerate(results):
                    ...
                    for j, box in enumerate(sorted_boxes):
                        ...
                        # Obtener sección de threshold para este ojo
                        eye_thresh = roi_thresh[ey-roi_y:ey-roi_y+eh, ex:ex+ew]
                        eye_color = self.frame_draw[ey:ey+eh, ex:ex+ew]
                        
                        if eye_thresh.size > 0:
                            eye_regions.append((eye_thresh, eye_color, ex, ey, ew, eh, is_first_eye))
                ...
                for data in eye_regions:
                    result = self.process_pupil(data)
                    if result:
                        ...
                        # Mostrar la máscara de threshold
