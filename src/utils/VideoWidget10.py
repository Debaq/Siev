...
class VideoProcesses:
    ...
    def process_eye_region(self, data):
        try:
            ...
            blur_size = 5  # Configurable: tamaño del filtro gaussiano
            eye_gray = cv2.GaussianBlur(eye_gray, (blur_size, blur_size), 0)


            use_clahe = True  # Configurable
            if use_clahe:
                # Configurable: clip limit y grid size
            ...
            if threshold_value == 0:
                ...
                otsu_thresh, _ = cv2.threshold(eye_gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
                adjusted_thresh = max(0, min(255, otsu_thresh + threshold_value))
                _, thresh = cv2.threshold(eye_gray, adjusted_thresh, 255, cv2.THRESH_BINARY_INV)
