...
class EyeDataProcessor:
    ...
    def _calculate_centers(self):
        ...
        if len(self.calibration_data['left']) > 0:
            ...
            logger.info(f"Calculated center for left eye: {self.left_eye_center}")
        ...
        if len(self.calibration_data['right']) > 0:
            ...
            logger.info(f"Calculated center for right eye: {self.right_eye_center}")
        
        # Marcar como calibrado
        self.is_calibrated = True
        logger.info("Calibration completed - Data will now be centered at 0")
