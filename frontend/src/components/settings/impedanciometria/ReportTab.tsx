...
const TEMPLATE_PRESETS: Record<string, ReportSection[]> = {
    ...
    minimal: [
        ...
        { id: 'patient', label: 'Datos del Paciente', enabled: true, order: 1 },
        { id: 'timpanograma', label: 'Timpanograma', enabled: true, order: 2 },
        { id: 'reflejos', label: 'Reflejos Estapediales', enabled: false, order: 3 },
        { id: 'clasificacion', label: 'Clasificación', enabled: true, order: 4 },
        { id: 'summary', label: 'Resumen Clínico', enabled: true, order: 5 },
        { id: 'signature', label: 'Firma', enabled: true, order: 6 },
    ],
...
export const ReportTab: React.FC<ReportTabProps> = ({ config, updateConfig }) => {
    const report = config.impedanciometria.report;

    const handleTemplateChange = (template: string) => {
        updateConfig('impedanciometria.report.template', template);
        if (template !== 'custom' && TEMPLATE_PRESETS[template]) {
            updateConfig('impedanciometria.report.sections', TEMPLATE_PRESETS[template]);
        }
    };
    ...
    const handleSectionsChange = (newSections: ReportSection[]) => {
        ...
        if (currentPreset) {
            ...
            if (!isSame) {
                updateConfig('impedanciometria.report.template', 'custom');
            }
        }
        updateConfig('impedanciometria.report.sections', newSections);
    };
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <SettingsSection
                ...
                description="Seleccione una plantilla predefinida o personalice las secciones del informe de impedanciometría."
            ...
            >
                ...
                <div className="text-xs text-dark-400 mt-2">
                    {report.template === 'standard' && 'Incluye timpanograma, reflejos y clasificación.'}
                    {report.template === 'detailed' && 'Informe completo con todas las secciones habilitadas.'}
                    {report.template === 'minimal' && 'Timpanograma y clasificación sin reflejos.'}
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <SettingsToggle
                            ...
                            onChange={v => updateConfig('impedanciometria.report.include_logo', v)}
                        />
                    </div>

                    <div className="flex items-center justify-between">
                        <div>
                            <div className="text-sm text-dark-200">Incluir Timpanograma</div>
                            <div className="text-xs text-dark-500">Mostrar la curva del timpanograma en el informe</div>
                        </div>
                        <SettingsToggle
                            checked={report.include_tympanogram}
                            onChange={v => updateConfig('impedanciometria.report.include_tympanogram', v)}
                        />
                    </div>

                    <div className="flex items-center justify-between">
                        <div>
                            <div className="text-sm text-dark-200">Incluir Tabla de Reflejos</div>
                            <div className="text-xs text-dark-500">Mostrar tabla de reflejos estapediales en el informe</div>
                        </div>
                        <SettingsToggle
                            checked={report.include_reflex_table}
                            onChange={v => updateConfig('impedanciometria.report.include_reflex_table', v)}
                        />
                    </div>
                </div>
            </SettingsSection>
            ...
            >
                <SettingsField label="Formato">
                    <select
                        ...
                        onChange={e => updateConfig('impedanciometria.report.export_format', e.target.value)}
                    >
