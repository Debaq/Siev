...
export const HardwareTab: React.FC<HardwareTabProps> = ({ config, updateConfig }) => {
    const { ports, connect, disconnect, isConnected, refreshPorts } = useTauriHardware(config);
            ...
            >
                <SettingsField label="Puerto Serial">
                    <div className="flex gap-2">
                        ...
                        <button className="btn btn-secondary p-2 h-9" onClick={refreshPorts}>
                            <RefreshCw className="w-4 h-4" />
                        </button>
                    </div>
                </SettingsField>

                <div className="flex items-center gap-4 p-3 bg-dark-800/50 rounded-lg border border-dark-700">
                    <div className="flex-1">
                        ...
                        <p className="text-[10px] text-dark-500">Máscara SIEV v1.0 detectada en {config.vng.hardware.serial_port || 'N/A'}</p>
                    </div>
                    {!isConnected ? (
                        <button 
                            className="btn btn-primary px-4 py-1.5 h-auto text-xs"
                        ...
                        <button className="btn btn-danger px-4 py-1.5 h-auto text-xs" onClick={disconnect}>
                            Desconectar
                        </button>
                    )}
                </div>
                ...
                <SettingsField label="Intensidad LEDs Infrarrojos">
                    ...
                    <p className="text-[10px] text-dark-500 mt-0.5">Ajuste la potencia de los LEDs IR para evitar reflejos o saturación.</p>
                </SettingsField>
            </SettingsSection>
        </div>
    );
